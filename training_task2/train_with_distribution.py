...
from promptmr_task2 import PromptMR
from mri_data_task2 import *
...
def log_to_tensorboard(writer, epoch, recons_pred, kspace_pred, target, fully_kspace, masked_kspace, mode_name):
    ...
    masked_kspace_magnitude = np.log(fastmri.complex_abs(masked_kspace).cpu().numpy() + 1e-16)
    fully_kspace_image_0 = fully_kspace_magnitude[0, 0, :, :]
    masked_kspace_image_0 = masked_kspace_magnitude[0, 0, :, :]
    ...
    masked_kspace_image_0 = (masked_kspace_image_0 - masked_kspace_image_0.min()) / (masked_kspace_image_0.max() - masked_kspace_image_0.min())
    ...
    masked_kspace_image_0 = torch.tensor(masked_kspace_image_0).unsqueeze(0)
    ...
    writer.add_image(f'{mode_name}/masked_kspace_0', masked_kspace_image_0, epoch)
...
def create_dataloader(fileName, batch_size, args, mode, train=True):
    ...
    if train:
        transform = PromptMrDataTransform(mask_func=None, use_seed=False)
    else:
        transform = PromptMrDataTransform(mask_func=None)
    ...
    return torch.utils.data.DataLoader(dataset, batch_size=batch_size, sampler=sampler, num_workers=args.num_workers)
...
def train_epoch(train_loader, model, optimizer, device, scaler, mode_name, epoch, writer):
    ...
    for i, batch in enumerate(tqdm(train_loader, desc=f"Training, {mode_name}")):
        ...
        fully_kspace, masked_kspace, mask, target = fully_kspace.float().to(device), masked_kspace.float().to(device), mask.float().to(device), target.float().to(device)
...
def validate(val_loader, model, device, mode_name,  writer, epoch):
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc=f"Validating, {mode_name}"):
            fully_kspace, masked_kspace, mask, target = batch
            fully_kspace, masked_kspace, mask, target = fully_kspace.float().to(device), masked_kspace.float().to(
                device), mask.float().to(device), target.float().to(device)
            ...
            if epoch % args.log_every_n_epochs == 0:
                log_to_tensorboard(writer, epoch, recons_pred, kspace_pred, target, fully_kspace, masked_kspace, mode_name)
...
def cli_main(args):
    ...
    fh = logging.FileHandler(os.path.join(args.experiments_output, 'train_task2_random5part.log'))
    ...
    part1_indices = {
        modality: list(range(0, len(train_loaders[modality][0].dataset) // 2)) for modality in modality_names
    }
    part2_indices = {
        modality: list(range(len(train_loaders[modality][0].dataset) // 2, len(train_loaders[modality][0].dataset))) for modality in modality_names
    }
    ...
    for epoch in range(args.max_epochs):
        ...
        for modality in modality_names:
            ...
            for train_loader in train_loaders[modality]:
                ...
                if epoch % 2 == 0:
                    current_train_indices = part1_indices[modality]
                else:
                    current_train_indices = part2_indices[modality]

                # Create a new DataLoader for the current part
                train_loader_part = torch.utils.data.DataLoader(
                    Subset(train_loader.dataset, current_train_indices),
                    batch_size=train_loader.batch_size,
                    sampler=DistributedSampler(Subset(train_loader.dataset, current_train_indices),
                ...
                train_loader_part.sampler.set_epoch(epoch)  # Ensure each epoch uses the appropriate subset
                train_loss = train_epoch(train_loader_part, model, optimizer, args.device, scaler, modality, epoch, writer)
                modality_train_losses.append(train_loss)

            for val_loader in val_loaders[modality]:
                val_loader.sampler.set_epoch(epoch)  # Ensure each epoch uses a different subset
...
def build_args():
    ...
    parser.add_argument("--data_path", default=pathlib.Path('/ari/users/rxu/MultiCoil'))
    parser.add_argument("--experiments_output", default=pathlib.Path('/ari/users/rxu/training_task2/output/distribution'))
    parser.add_argument("--aorta_batch_size", default=6, type=int)
    parser.add_argument("--cine_lax204_batch_size", default=4, type=int)
    parser.add_argument("--cine_lax168_batch_size", default=6, type=int)
    parser.add_argument("--cine_sax162_batch_size", default=4, type=int)
    parser.add_argument("--cine_sax204_batch_size", default=4, type=int)
    parser.add_argument("--cine_sax246_batch_size", default=2, type=int)
    parser.add_argument("--cine_lvot_batch_size", default=6, type=int)
    parser.add_argument("--t1map_batch_size", default=6, type=int)
    parser.add_argument("--t2map_batch_size", default=10, type=int)
    parser.add_argument("--tagging_batch_size", default=4, type=int)
    parser.add_argument("--gpus", default=4, type=int, help="Number of GPUs to use")
    parser.add_argument("--device", default='cuda')
    parser.add_argument("--h5py_folder", default='TrainingSet/h5_FullSample', type=str, help="folder name for converted h5py files")
    parser.add_argument("--num_workers", default=12, type=int, help="Number of workers to use in data loader")
    parser.add_argument("--lr", default=0.0001, type=float, help="Adam learning rate")
    parser.add_argument("--lr_step_size", default=1, type=int, help="Epoch at which to decrease step size")
